...
class Storage:
    ...
    def delete_event_type(self, event_type_id: str) -> bool:
        with self._lock:
            if self.event_types.pop(event_type_id, None) is None:
                return False
            self.bookings = [b for b in self.bookings if b["eventTypeId"] != event_type_id]
            return True
    ...
    def set_working_hours(self, config: WorkingHoursConfig) -> WorkingHoursConfig:
        with self._lock:
            self.working_hours = config.model_copy(deep=True)
            return self.working_hours.model_copy(deep=True)
